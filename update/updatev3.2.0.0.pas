...
g:=0;
 ...
 myinifile:=Tinifile.Create(path+'\LocalVerson.ini');
      StrIni:=myinifile.Readstring('version','info','');//读取本地版本
      edit2.Text:=  StrIni;
      StrLocal:=  StrIni;
     form1.Caption:='枫叶香蕉v'+ StrIni;
       StrInfor:=IdHTTP1.Get('server version fetch');   //读取服务器 版本信息
         EDIT1.Text:=StrInfor;
         StrServer:= StrInfor;
...
StatusBar.Canvas.Font.Color:=clyellow;//字體顏色
...
myinifile.Destroy;
 TZipFile.ExtractZipFile(StrObject+'update.zip',StrObject);
                 ...
                 StatusBar1.Panels[3].Text:='更新完成,正在重启你的程序！';
                 sleep(2000);
         ShellExecute(0, nil, 'goagent', nil, pchar(StrObject), SW_SHOWNORMAL); //  启动goagent
         sleep(2000);
       if CheckTask('goagent.exe') then
       begin
       StatusBar1.Panels[3].Text:='启动程序成功' ;
       sleep(3000);
       halt;
       ...
       begin
         StatusBar1.Panels[3].Text:='启动失败,请手动启动程序';
       end;
       end;
